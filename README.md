...
> 说明：PDF 输出为图片渲染（不含文字层——所谓文字是像素）；PDF 输入仅提取文本（不保留版式与图片）；PDF → 图片逐页渲染，多页文档下载为按所选格式（PNG / JPEG / WebP）逐页出图的 ZIP 包；BMP、GIF、SVG 仅支持作为输入——浏览器无法编码它们，GIF 转成图片或 PDF 时只保留第一帧（源文件确有多帧时，结果卡片上也会说明）；Markdown 或 SVG 转成 Word / Markdown 时，内联 SVG 会先栅格成 PNG 再嵌入——图形保住了，但它是位图不是矢量（结果卡片上也会说明）；多工作表 XLSX → CSV 会下载包含"每表一个 CSV"的 ZIP 包。上面三组的计数口径：48 条注册路径经 BFS 可到达 143 个源→目标组合，其中 27 个因语义无效被屏蔽（24 个图片 → TXT / CSV / JSON / XLSX，3 个 PDF → CSV / JSON / XLSX），因此目标格式下拉实际提供 116 个组合。

**逐条链路的取舍，各写成一页。** 每条链路单独一页，写清它实际执行几步、哪些结构会保留、哪些会丢以及为什么丢：
[Markdown 转 Word](https://liaolongdong.github.io/transfer-any-file/convert/markdown-to-word.html) ·
[Word 转 Markdown](https://liaolongdong.github.io/transfer-any-file/convert/word-to-markdown.html) ·
[Word 转 PDF](https://liaolongdong.github.io/transfer-any-file/convert/word-to-pdf.html) ·
[Markdown 转 PDF](https://liaolongdong.github.io/transfer-any-file/convert/markdown-to-pdf.html) ·
[PDF 转纯文本](https://liaolongdong.github.io/transfer-any-file/convert/pdf-to-text.html) ·
[Excel 转 CSV](https://liaolongdong.github.io/transfer-any-file/convert/excel-to-csv.html) ·
[CSV 转 Excel](https://liaolongdong.github.io/transfer-any-file/convert/csv-to-excel.html) ·
[JSON 转 CSV](https://liaolongdong.github.io/transfer-any-file/convert/json-to-csv.html) ·
[PNG 转 WebP](https://liaolongdong.github.io/transfer-any-file/convert/png-to-webp.html) ·
[SVG 转 PNG](https://liaolongdong.github.io/transfer-any-file/convert/svg-to-png.html)。
另有[转换总览](https://liaolongdong.github.io/transfer-any-file/convert/)（含完整格式矩阵）与博客长文[我把「文件格式转换」做成了一个不联网的 Chrome 扩展](https://liaolongdong.github.io/transfer-any-file/blog/)。
...
- **PDF 页码范围** — 批次里有 PDF 且目标为图片时，输出参数区多出一个「页码范围」：填 `1-3, 5` 就只栅格化那几页（`-`、`–`、`~` 三种连接号与 `,`、`，`、`、`、`;`、`；` 五种分隔符都认，留空是全部页）。它说的是「这份文件的哪几页」，不是「输出该长什么样」，所以**既不写存储、也不进预设**——存下来就会在下次打开时悄悄截断另一份无关的文档。一页都匹配不上时这一批明确失败，而不是把全部页当作「没设过」交回来；ZIP 里的文件名沿用原始页码，所以只转 2、4 页拿到的是 `page-2.png` 与 `page-4.png`
...
- **切走的标签页也有信号** — 批次结束时页面不在前台，标签页标题前缀「[完成]」，回到页面即恢复；不需权限，也不依赖上面那个默认关闭的开关
- **可中断** — 长批次可中途停止，已完成的文件保留；结果区会明确说明这一批「已取消」，而不是把没跑完的文件当成转换失败
- **增删文件不丢已有结果** — 往批次里追加文件、或删掉其中某一个，只影响它自己那一条结果：其余文件转好的仍在屏上，选中的目标也一起保留。只有当调整后批次里出现了转不到当前目标的文件，才清空整批并说明原因（混批只提供对全部文件都有效的目标）。换目标格式仍然是整体清空——那一屏结果是上一个目标的产物
...
- **输出文件名模板** — 结果默认叫 `原名_20260914_153012.pdf`：日期加时间精确到秒，同一天转两批不会互相覆盖。偏好设置里可以换成自己的写法，`{name}`（源文件名去掉后缀）、`{date}`、`{time}`、`{index}`（本批内序号，从 1 起）、`{target}`（目标格式）随取随用，输入框下面实时显示示例。打错的占位符（比如 `{tile}`）按普通文字原样留在名字里并当场提示，而不是悄悄少一截；`/`、`\`、`:`、`*`、`?`、`"`、`<`、`>`、`|` 与控制字符一律清掉，所以模板再离谱也生不出路径；渲染出的名字超过 120 个字符则按字符截断（文件系统按字节算，中文一字三字节，120 字对任何文字都还在限内）。扩展名不归模板管，始终是产物实际的那个（多步链路取最后一步，打包下载就是 `.zip`）；清空输入框回到默认
...
- **键盘与读屏可达** — 提供跳转主内容的 skip link、可见焦点环，完整支持 `prefers-reduced-motion`（这一档压掉的不只是时长：位移与缩放令牌一起归零，界面上不再有任何一处移动——Element Plus 的弹窗与下拉也在其中，它们的入场偏移是在减弱档里重写成「不位移」，而不是把时间压快）；转换进度与结果计数、文件列表的载入与清空都写在 `role="status"` 区域里，对照视图的分栏条带 `role="separator"` 与当前比例
...
2. 将文件或整个文件夹拖到页面任意位置（不必精准命中上传区），或点击选择、直接粘贴剪贴板内容
3. 选择目标格式——下拉框只会列出所有已选文件都能到达的格式；需要多步时，下方会显示实际路径（如 `MD → HTML → PDF`）
4. 转成图片时，选择器下方会出现**输出参数**——最长边、质量、体积上限、PDF 清晰度；源里有 PDF 还多一个**页码范围**（如 `1-3, 5`，留空是全部页）；一项都不设就是编码器原本的默认值
5. 点击 **开始转换**，随后单个下载或整批打包为 ZIP
6. 每次都做同一套转换？把它存成**预设**，下次在预设卡片里点一下，格式和参数一起回来
7. 打开右上角 **偏好设置**，可切换主题 / 语言 / 显示模式，开关完成通知与大批次确认，改写结果文件名模板（如 `{name}_{index}`），或重新绑定转换快捷键
...
不能无损做到。PDF 输入只提取文本，PDF 输出是逐页渲染成图片，因此转换得到的 PDF 不含文字层、文字不是真实字符。Word 与 Excel 之间以 HTML 作为枢纽格式。

**为什么不能把截图转成文本文件？**
那需要 OCR，而项目没有内置任何 OCR 引擎——它会带来几十 MB 体积和一次模型下载，与"完全离线"的承诺冲突。所以图片 → TXT/CSV 是置灰并给出原因，而不是等到转换时才失败。

**图片转成的 PDF，为什么还能选中、复制出文字？**
那是 PDF 查看器做的，不是本扩展。转出的 PDF 是逐页位图，文件里既没有文字层也没有真实字符；部分查看器会在展示时自行识别图上的文字，于是你能选中并复制出字符——识别发生在查看器那一侧，识别结果也不在这个文件里。本扩展不内置 OCR，既读不到也写不出这些字符，所以图片 → TXT / CSV 依旧置灰。批次里出现 PDF 时，结果卡片上也会写明这一点。
...
**结果文件名的时间戳能去掉吗？**
能。偏好设置里的「输出文件名」默认写法是 `{name}_{date}_{time}`，改成 `{name}` 就得到 `report.pdf` 这样干净的名字。代价是同批里两个文件可能撞到同一个名字——这时后一个自动带 `_2`、`_3` 后缀，这道防线一直在，不管模板里有没有 `{date}`。改动只影响之后下载的产物；历史里记的仍是源文件名，改名不会把某条历史变成搜不到。
