...
/**
 * Every HTML page under `docs/<dir>/`, as DOCS entries.
 *
 * These are generated by `scripts/render-site-pages.mjs`, and `pnpm pages:check` already refuses to
 * let the committed HTML drift from its data source — but that check compares bytes, so it cannot
 * notice the data source itself quoting a stale 46. Enumerating the directory rather than naming the
 * files keeps a new pair page in scope the moment it is rendered.
 */
function generatedPages(dir) {
  const absolute = path.join(ROOT, 'docs', dir);
  if (!fs.existsSync(absolute)) {
    console.error(`prose numbers: docs/${dir}/ is missing — run \`pnpm pages:render\` before the check.`);
    process.exit(1);
  }
  return fs
...
const DOCS = [
  ...
  // these pages need the fact check as much as the hand-written ones do.
  'docs/blog/index.html',
  ...generatedPages('convert'),
  'docs/promo/wechat-article.md',
  'docs/promo/blog-article.en.md',
  'docs/promo/weibo-posts.md',
  // Channel copy carries the same claims to dev.to / HN / Reddit / Juejin, and it is written from a fact
  ...
  // The visibility runbook quotes the same closure counts, thresholds, store field lengths and package
