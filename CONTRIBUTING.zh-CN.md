...
2. **改代码，同时保证那些承诺依然成立**
   ...
   pnpm verify:meta      # package.json / wxt.config.ts / .github/repo-metadata.json 三者一致
...
- **文档**：面向用户的改动要同时更新 `README.md` **和** `README.zh-CN.md`；商店文案在 `CHROMEWEBSTORE.md`；产品说明页是 `docs/index.html`。值得发布的改动要在 `CHANGELOG.md` 与 `CHANGELOG.zh-CN.md` **各**记一条，版本号等于 `package.json#version`（发布 tag 也必须与之一致）。仓库的 GitHub About 描述改在 `.github/repo-metadata.json` 里，**不要**只在页面上手改，这样它才受版本管理并被 `pnpm verify:meta` 校验。安全漏洞走 `SECURITY.md`，不是 issue。安全策略与贡献须知本身也有中文对：`SECURITY.zh-CN.md` / `CONTRIBUTING.zh-CN.md`，改英文正文时同步改。UI 变更后跑 `pnpm assets:capture` 重新生成截图，避免素材与实际界面漂移。
...
一旦提交贡献，即表示你同意这些贡献按项目的 [ISC 许可证](LICENSE)发布。
