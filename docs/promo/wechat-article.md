...
- 一堆成熟的纯前端库：`marked`（Markdown → HTML）、`turndown`（HTML → Markdown）、`mammoth`（.docx → HTML）、`pdf.js`（PDF → 文本/图像）、`SheetJS`（.xlsx ⇄ CSV/JSON）、`jsPDF` + `html-to-image`（HTML → PDF/PNG）、`html-docx-js-typescript`（HTML → .docx）、`fflate`（ZIP）、`file-saver`（落盘）、`DOMPurify`（净化）。

把这些拼起来，就能覆盖办公场景里最常见的那 14 种格式：Markdown、HTML、Word(.docx)、PDF、TXT、CSV、Excel(.xlsx)、JSON、PNG、JPEG、WebP、BMP、GIF、SVG。

边界同样清楚，而且我不会假装它不存在：

- **没有 OCR。** 把截图变成文字需要一个模型，几十 MB 起步、还要联网下载——这直接违背"离线"这个前提，所以图片 → TXT/CSV/JSON/XLSX 在界面上是**置灰、悬停时写明原因**，而不是等你点了转换才报错。
...
但"支持多少个转换"这个问题，只有一个诚实的答案：**图上可达 143 个，界面实际提供 116 个**。差的那 27 个由一层策略（`utils/core/conversion-policy.ts`）置灰——它们在图上是通的，语义上却没有意义：图片 → TXT/CSV/JSON/XLSX 共 24 个（栅格图里没有文字层，除非接 OCR），PDF → CSV/JSON/XLSX 共 3 个（PDF 不带表格结构）。这些边不能直接删掉，因为同一条边换源格式就成立了（HTML → JSON 面对真实表格完全可用），所以只能在这一层按"源 + 目标"的组合屏蔽，并给出置灰原因。
...
**混合源格式批次。** 拖进来的可能是 .md、.csv、.xlsx 各若干。下拉框照样列出全部格式，但**只有对每一个文件都可达的目标保持可选**，其余置灰——否则用户选了个只有部分文件能转的目标，一半会失败。
...
**首屏不该背着 pdf.js。** 所有重型依赖在转换器内部动态 `import()`，重型子组件用 `defineAsyncComponent` 懒加载。整个扩展产物 3.71 MB，但打开工作台时不会去拉 pdf.worker（1.26 MB）。

**让重复动作变便宜。** 有一批细节只有天天用才会察觉：最近用过的 6 个目标格式会以下拉顶部的「最近使用」分组呈现，并且只对当前批次仍然可选的那些出现；拖放区不限于那块虚线框，整个页面都能接住文件（转换进行中投放被忽略，光标显示为禁止）；剪贴板里的内容直接 `⌘V` / `Ctrl+V` 就能转；上一轮结果可以撤销；开始转换的 `Ctrl/⌘ + Enter` 可以改绑；分隔条位置和历史卡片的折叠状态跨会话保留；长批次在后台标签页跑完时，可以选择发一条桌面通知。

**无障碍不是慈善项目，是可测的指标。** 两组成对的颜色从真实页面上读回来，在 6 种主题 × 明暗共 12 组配置下逐主题断言：顶栏品牌文字对顶栏底色 ≥ 4.5:1（WCAG 2.1 AA 对文本的要求），焦点环对卡片底色 ≥ 3:1（AA 对界面组件的要求）；测试还单独断言第一次 Tab 落在 skip link 上、并且它显示可见的焦点环——不是"看着还行"。这里也留着一个已知缺陷：浅色的森林绿 / 活力橙主题下，主按钮只有 2.5–3.6:1，因为它的 base / hover / active 三档底色没法被同一种前景同时覆盖，要修得重排浅色主色阶。写在这里，免得别人替我发现。

**「离线」这个说法要划清可证的边界。** CI 里 `pnpm verify:offline` 断言的是：第一方源码不含任何网络调用，manifest 只要 `storage`、没有 `host_permissions`。它**不断言**打包产物里搜不到 `fetch` 字符串——jsPDF 和 pdf.js 的未调用路径里确实带着。这些路径不会被第一方代码走到；即使走到了，在没有 host 权限、也没有 content script 的情况下，扩展读不到响应、也碰不到任何网站的数据。"0 个网络请求"只有按这个说法写才站得住，也才值得写出来。
