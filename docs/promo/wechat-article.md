...
> 14 种格式、48+ 条转换路径、0 个网络请求。这篇文章讲清楚三件事：为什么在线转换器不适合处理你的敏感文件、浏览器为什么其实已经够用、以及做成本地工具会撞上哪些真实的工程取舍。
...
- **PDF 输出是图片渲染**，转换得到的 PDF 不含文字层；PDF 输入只提取文本，版式和图片不保留。
...
结果有两个。第一，**多步链路是"求出来的"，不是写出来的**——用户点一次"Markdown 转 PDF"，实际跑了两段。第二，**新增一个转换器只有 20 行**，而所有经过它的路径立刻自动可用。当前这张图有 48 条直连边，工作台页脚那行"14 formats supported, 48+ conversion paths"就是从图里现算出来的，不是文案。
...
**首屏不该背着 pdf.js。** 所有重型依赖在转换器内部动态 `import()`，重型子组件用 `defineAsyncComponent` 懒加载，连打包用的 fflate 也是到真正要用的那一刻才加载。整个扩展产物 3.76 MB，但打开工作台时不会去拉 pdf.worker（1.26 MB）。
...
**「转成 WebP」只是半个需求。** 目标是图片时，工作台多出一行输出参数：质量、最长边，以及一个目标体积——编码器先把质量往下走，走不动了才减像素，所以体积是**追到的上限，不是承诺**。四条能产出图片的链路（图片→图片、SVG→图片、HTML→图片、PDF→图片）走的是同一个编码出口，因此「质量 70%」不会在四条路上变成四种意思。如果这件事每周都要做一次，目标连同参数可以存成一张命名预设卡片，一键恢复；而且预设绑的是**目标**不是源格式——「PNG → 200 KB WebP」这一张，下次拖进来的不管是 Markdown、PDF 还是 PNG 都能用。

**无障碍不是慈善项目，是可测的指标。** 四组成对的颜色从真实页面上读回来，在 6 种主题 × 明暗共 12 组配置下逐主题断言：顶栏品牌文字对顶栏底色 ≥ 4.5:1，主按钮文字在静止 / 悬停 / 按下三种状态下对按钮底色 ≥ 4.5:1（这两项是 WCAG 2.1 AA 对文本的要求），焦点环对卡片底色 ≥ 3:1（AA 对界面组件的要求），拖放提示、文件大小、页脚这些正文信息文字对各自底色 ≥ 4.5:1（1.4.3 豁免真正的输入占位符、禁用控件和状态已由 ARIA 承载的装饰，豁免不了给人读的句子）；测试还单独断言第一次 Tab 落在 skip link 上、并且它显示可见的焦点环——不是"看着还行"。主按钮这一条最值回票价：先把它写成测量时，12 组里有 7 组不达标，最差 3.45:1，而且失败点全在悬停或按下的中间色阶上，静止态反而是过的——修法是让这两档色阶从文字颜色的反方向推导，不再跟着主题的明度阶梯走。今天最差的一组是 4.70:1（浅色玫瑰红），深色六档都在 7.03:1 以上。还有一个缺口公开写着而不是藏起来：控件的填充色对它所在的底色需 ≥ 3:1（WCAG 1.4.11），12 组里成立 10 组，浅色的森林绿与活力橙不成立（最浅状态 2.21 / 2.96:1）——这两档填充本身就接近白色，再加深就要花掉文字那一侧的余量。
...
技术栈：**WXT + Vue 3 + TypeScript + Element Plus**，Manifest V3，权限只有 `storage`，MIT 协议。代码里的验证方式也一并交代：Playwright 驱动构建产物跑 254 条断言，夹具（`fixtures/`）随仓库走。
