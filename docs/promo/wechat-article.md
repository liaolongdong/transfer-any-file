...
**无障碍不是慈善项目，是可测的指标。** 三组成对的颜色从真实页面上读回来，在 6 种主题 × 明暗共 12 组配置下逐主题断言：顶栏品牌文字对顶栏底色 ≥ 4.5:1，主按钮文字在静止 / 悬停 / 按下三种状态下对按钮底色 ≥ 4.5:1（这两项是 WCAG 2.1 AA 对文本的要求），焦点环对卡片底色 ≥ 3:1（AA 对界面组件的要求）；测试还单独断言第一次 Tab 落在 skip link 上、并且它显示可见的焦点环——不是"看着还行"。主按钮这一条最值回票价：先把它写成测量时，12 组里有 7 组不达标，最差 3.45:1，而且失败点全在悬停或按下的中间色阶上，静止态反而是过的——修法是让这两档色阶从文字颜色的反方向推导，不再跟着主题的明度阶梯走。今天最差的一组是 4.70:1（浅色玫瑰红），深色六档都在 7.03:1 以上。还有一个缺口公开写着而不是藏起来：控件的填充色对它所在的底色需 ≥ 3:1（WCAG 1.4.11），12 组里成立 10 组，浅色的森林绿与活力橙不成立（最浅状态 2.21 / 2.96:1）——这两档填充本身就接近白色，再加深就要花掉文字那一侧的余量。
