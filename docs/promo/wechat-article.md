...
**首屏不该背着 pdf.js。** 所有重型依赖在转换器内部动态 `import()`，重型子组件用 `defineAsyncComponent` 懒加载，连打包用的 fflate 也是到真正要用的那一刻才加载。整个扩展产物 3.77 MB，但打开工作台时不会去拉 pdf.worker（1.26 MB）。
...
技术栈：**WXT + Vue 3 + TypeScript + Element Plus**，Manifest V3，权限只有 `storage`，MIT 协议。代码里的验证方式也一并交代：Playwright 驱动构建产物跑 292 条断言，夹具（`fixtures/`）随仓库走。
