...
**Chinese CSV is an encoding problem, not a language problem.** Read: try UTF-8, fall back to GBK. Write: prepend a UTF-8 BOM. Skip that and Excel opens the file as mojibake, and Chinese-speaking users churn inside the first minute.
...
**The first screen shouldn't carry pdf.js.** Every heavy dependency is dynamically `import()`ed inside its converter, and heavy child components are lazy-loaded with `defineAsyncComponent`. The whole extension builds to 3.71 MB, but opening the workbench never pulls `pdf.worker` (1.26 MB).

**Make repeated actions cheap.** A set of details you only notice after daily use: your 6 most recent target formats appear as a _Recently used_ group at the top of the picker, and only while they're still selectable for the current batch; dropping files works anywhere on the page, not just inside the dashed zone (a drop during a running conversion is ignored, with a forbidden cursor); clipboard contents convert with `⌘V` / `Ctrl+V`; the previous batch can be undone; the `Ctrl/⌘ + Enter` convert shortcut is rebindable; the split-divider position and the history card's collapsed state persist across sessions; and a long batch finishing in a background tab can optionally raise a desktop notification.

**Accessibility is a measurable number, not a goodwill gesture.** Two contrast pairs are read back off the live page in all 6 accent themes × light and dark — 12 configurations — and asserted per theme: topbar brand text against the topbar at 4.5:1 or better, and the focus ring against a card at 3:1 or better (WCAG 2.1 AA for text and for user-interface components respectively). The suite separately asserts that the first Tab lands on the skip link and that it shows a visible ring. Not "looks fine". One known gap stays open: on the light forest-green and light orange themes the primary button is only 2.5–3.6:1, because its base / hover / active backgrounds can't all be covered by one foreground; fixing it means re-deriving the light primary scale. Documenting it here is cheaper than having someone else find it.
...
Tech stack: **WXT + Vue 3 + TypeScript + Element Plus**, Manifest V3, `storage` as the only permission, ISC licence. How it's verified is part of the deliverable too: Playwright drives the built artifact through 159 assertions, and the fixtures it converts live in `fixtures/` next to the code.
