...
- `Conversion Cancel` 那小节的分支数不恒定（4/3/1）与它本轮的超时，根因已定位：一段长同步转换会
  饿死 Playwright 的页内等待器，`polling: 'mutation'` 同样（它也按帧触发），所以那 2 秒窗口能不能抓到
  `.cancel-btn` 由机器负载决定。**修法要动既有测试的分支结构**，等于改一份对外契约的取证方式，
  所以列在这里等决定，没有顺手改。
