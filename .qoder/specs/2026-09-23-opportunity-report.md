...
## 17. 追加收口：轮次 E（启动期重复读取 + 那段从来没生效的串行）（`0b93575`）

**模拟器的 `get` 是微任务，真 Chrome 是下一个任务——不补这层包装，新写的那条断言是装饰。**
`mockChromeStorage()` 用 `Promise.resolve` 兑现 `get`、同步落库 `set`，于是每条没串起来的写入链
各自在自己的定时器回调里跑完，天然自串行：把 `persistQueue` 的链接整段删掉，那一节照样绿。
改法不是调大等待，而是**给这一页的 `get` 换成 Chrome 的形状**——按派发那一刻取值、下一个任务才送到
（`setTimeout(..., 30)`），只加在 `cardPage` 上，不碰共享 mock 以免影响别的小节。30 ms 是试出来的：
0 ms 时未串行的实现仍然绿（等于没测），30 ms 时它恰好丢掉 `presets`、只留 `history`，也就是这个 bug
本来的形状。写这条的通用教训：**异步断言的"有牙"取决于替身的时序形状，不取决于等待时长**；一条
只等时间的回归测试，先用"把修复删掉看它是否变红"来验收，删掉还绿就是测试写错了。

**可归因导出树会被"整份 cp 工作区文件"污染，而且症状指向别人。** 最后那次同步我把工作区的
`scripts/e2e-test.mjs` 整个拷进 `/tmp/taf-roundF`，于是并发会话那节 F5（输出文件名模板）跟着进了
树，而它的源码（`utils/core/name-template.ts`、`useNameTemplate.ts`、`PreferencesMenu` 的输入框）
没进——跑出来 `304/305`，唯一那条红是 `✗ Output file names follow a template: page.$eval: Failed to
find element matching selector ".name-input input"`。这一行看起来像我的改动弄坏了什么，实际是我的
树不干净。修法固定下来：导出树的同步一律走 `git diff -U3` + 按 hunk 过滤 + `patch -p1`，不用 `cp`；
并且每轮跑之前做一次**全树等价证明**（HEAD 导出树逐文件比对，允许的差异只列本轮那几份）。这一轮
据此把 `CHANGELOG.md` / `.en.md` 里属于并发会话的那两格也删掉了——`verify:numbers` 会把整段
`[未发布]` 读进取证范围，带着别人的卖点句子跑出来的绿灯不算我的绿灯。

**后台命令的"completed exit 0"两次都不是结论。** 一次是 `timeout` 上限（10 分钟）到点把跑了一半的
套件截断，日志停在 144 行、既无 `Results` 横幅也无退出码，通知照样报 0；另一次通知来得比进程结束
早，`ps` 一看 node 与 chrome 还在跑（那次最终跑出 303/303）。所以这一轮起认的凭证只有日志里自己
`echo` 出来的 `E2E_EXIT=` 与 `Results: N/N` 横幅，退出码一律重定向到文件而不是从通知读。
同一轮还记一次 `pnpm $s | tail` 的老坑：那样 `echo $?` 拿到的是 `tail` 的退出码，九条门禁全部
"通过"是假的——改成每条单独落文件、先取 `code=$?` 再 tail。

**`<script setup>` 顶层的 `let` 是每个实例一份，产物里能直接看见。** 折叠写入那条串行 2026-09-20
就写进过 CHANGELOG，但它声明在 `setup()` 里，于是每张卡片只跟自己串行，跨卡片照样读—改—写交错，
后写的用手里那份旧映射覆盖先写的键。没有一条测试能察觉，因为一张卡片就足够通过。这轮把全仓
`<script setup>` 顶层的 `let` 逐条过了一遍：防抖计时器、`isMounted`、`paintObserver` 这些都是
**要的**就是每实例语义，只有那一处是要跨实例的。判据写进新模块的注释里（`utils/core/collapsed-state.ts`
第 6-9 行），下次有人想在组件里放跨实例状态，先撞见这段话。

**评审挖出的那条窄口径差异，按"影响交互就先停"留在门口。** seed 之后「第一次点开偏好」不再是
跨标签页的追平点：旧 `initTheme` 那一次重读顺带读到别的标签页改过的主题并立即应用，现在这一次读
不发生了。要同时开着两个工作台标签页、且在第一次点开偏好之前于另一个标签页改过主题才碰得到，
而 `chrome.runtime.openOptionsPage()` 通常又会聚焦已开的标签页。把它做成"启动即注册 storage
监听"能同时拿到更少读与更实时，但那是一次**可见的交互变化**（没人点开偏好也会换肤），所以只写进
commit message 与这一节，不动代码，等一句确认。同类仍等授权的还有三条：`v-show`→`v-if`、
`useConversion` 并发、`pdf-to-image` 流式 ZIP。

**验证与体积**：可归因树 = `4654000` + 本轮 11 份文件（`git hash-object` 与暂存 blob 逐个全等），
完整套件 **303/303 exit 0**，断言基线由这轮自己写下 302→303，同步进四处已跟踪载体；
`lint:all` 与 meta / numbers / paths / listing / offline 两层 / remote-code 两层在同一棵树上全部
exit 0（`pages:check` 仍跑不了：渲染脚本与 `docs/convert/` 未纳入版本管理，属并发会话那笔欠账）。
整包 3,770,842 → **3,771,159 B**、首屏 434,124 → **434,441 B**，两处都是 **+317 B**——新增一个
模块的代价，换来启动窗口少一读、偏好面板首次弹出少两读、以及一段真的会串行的写入。这一轮的绿灯
是在 1 分钟负载 236–441 之间拿下的（本机常驻 jekyll serve 与 IDE 各自占到 30%+ CPU），而同一棵树
在上一轮 load ~300 时红过两条重节点——超时预算这轮没动，所以这只能算"没复现"，不算"已解决"。
HEAD 在这一轮之内移动两次（`a38bbab` → `4654000`），302 那一格正是他们中间那笔带来的，
"同一棵树"的定义因此必须在提交前重新取一次，不能沿用一小时前的证明。未推送：本分支领先
`origin/feat/p0-data-integrity` 30 笔、领先 `origin/main` 89 笔（§16 那句"未推送 87 笔"是对着
`origin/main` 的当时的数）。
