...
1. 用 `XLSX.utils.csv_to_sheet(text, { raw: true })` 解析，使每个字段落成**字符串单元格**，
   保留引号与内嵌换行的正确语义。
   **实现前必须先实测** `raw` 在 `csv_to_sheet` 上确实是「不做类型推断」；若该选项不成立，
   退回手工按 RFC 4180 切分并构造 `{t:'s'}` 单元格。
2. 逐格判定是否转数字，**以往返检查为唯一判据**：`/^-?(?:0|[1-9]\d*)(?:\.\d+)?(?:[eE][+-]?\d+)?$/`
   且 `String(Number(v)) === v`。前导零、`1.50`、`1e5` 会被往返检查自然挡下，
   **不要另写条件分支**（`00424`、`01.50`、`1e5` 保持文本；`1234.5` 转数字）。
3. 其余一律 `{ t: 's' }`，并设 `z: '@'` 文本格式，避免 Excel 打开后二次重解析。
4. 写前对文本单元格跑 `guardCsvValue`（`csv-guard.ts:15`）。
