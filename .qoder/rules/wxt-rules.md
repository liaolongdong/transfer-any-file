...
- 按改动范围执行：TS/Vue/运行时 → `pnpm lint:all`；入口/manifest/依赖/打包 → `pnpm build`；转换逻辑或端到端行为 → `pnpm test:e2e`（Playwright + `fixtures/`）；商店文案 `CHROMEWEBSTORE.md` → `pnpm verify:listing`；转换器注册与路由 → `pnpm verify:paths`；对外文档里的数字 → `pnpm verify:numbers`；生成站点页（`docs/convert/**` 与 `docs/sitemap.xml`）→ `pnpm pages:check`（文案只改数据源 `scripts/conversion-pages/pairs.mjs`，再 `pnpm pages:render`，手改生成物会被下一次渲染覆盖）。
